...
public class FakeImageReaderTests
{
    ...
    public async Task Records_Glossary_And_ImageCount()
    {
        ...
        var text = await r.ExtractAsync(new[] { Img(), Img() }, new List<string> { "Obersaxen" }, CancellationToken.None);
    ...
    public async Task Throws_When_Configured()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => r.ExtractAsync(new[] { Img() }, Array.Empty<string>(), CancellationToken.None));
    }
}
