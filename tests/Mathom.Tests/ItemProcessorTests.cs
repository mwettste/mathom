...
public class ItemProcessorTests(PostgresFixture fx)
{
    ...
    public async Task ProcessAsync_FillsFieldsAndTags_SetsReady()
    {
        ...
        var fake = new FakeLlmClient
        {
            Respond = _ => new CleanupResult(
                ...
                new[]
                {
                    new CleanupTag("projects", TagKind.Topic),
                    new CleanupTag("thing", TagKind.Project),
                }
            ),
        };
        ...
        await using (var db = fx.NewDbContext())
        {
            var processor = new ItemProcessor(
                ...
                new FakeImageReader(),
                new FakeMediaStore(),
                new Mathom.Web.Media.PhotoVariantService(
                    db,
                    new FakeMediaStore(),
    ...
    public async Task ProcessAsync_Voice_OnTranscriptionFailure_SetsFailed()
    {
        ...
        await using (var db = fx.NewDbContext())
            await new ItemProcessor(
                db,
                new FakeLlmClient(),
                new FakeTranscriber { Throw = true },
                new FakeImageReader(),
                media,
                new Mathom.Web.Media.PhotoVariantService(
                    db,
    ...
    public async Task Reprocess_ReconcilesTags_NoDuplicateNoStale()
    {
        ...
        await using (var seed = fx.NewDbContext())
        {
            var item = new Item
            {
                ...
                ProcessedAt = System.DateTimeOffset.UtcNow,
            ...
            item.ItemTags.Add(
                ...
                {
                    Item = item,
                    Tag = new Tag { Name = "old1", Kind = TagKind.Topic },
                }
            );
            item.ItemTags.Add(
                new ItemTag
                {
                    Item = item,
                    Tag = new Tag { Name = "old2", Kind = TagKind.Topic },
                }
            );
            itemId = item.Id;
            seed.Items.Add(item);
        ...
        var llm = new FakeLlmClient
        {
            Respond = raw => new CleanupResult(
                ...
                ItemType.Note,
                false,
                new System.Collections.Generic.List<CleanupTag>
                {
                    new("old2", TagKind.Topic),
                    new("new1", TagKind.Topic),
                }
            ),
        };
        ...
        Assert.Equal(
            new[] { "new1", "old2" },
            item2.ItemTags.Select(it => it.Tag.Name).OrderBy(n => n).ToArray()
        );
    }
    ...
    public async Task Process_AppliesVariantCorrection_ToCleanOutput()
    {
        ...
        await using (var seed = fx.NewDbContext())
        {
            ...
            term.Variants.Add(
                new Mathom.Web.Domain.GlossaryVariant
                {
                    Id = System.Guid.NewGuid(),
            ...
            var item = Mathom.Web.Domain.Item.CreatePending(
                Mathom.Web.Domain.SourceType.Text,
                "Meeting Fairstills today",
        ...
        var llm = new FakeLlmClient
        {
            Respond = raw => new Mathom.Web.Processing.CleanupResult(
                raw,
                raw,
                Mathom.Web.Domain.ItemType.Note,
                false,
                new System.Collections.Generic.List<Mathom.Web.Processing.CleanupTag>
                {
                    new("Fairstills", Mathom.Web.Domain.TagKind.Topic),
                }
            ),
        };
        ...
        var processor = new Mathom.Web.Processing.ItemProcessor(
            ...
            Microsoft
                ...
                .Logging
                .Abstractions
                .NullLogger<Mathom.Web.Processing.ItemProcessor>
                .Instance
        );
    ...
    public async Task Process_PassesUsersGlossary_ToCleanup()
    {
        ...
        await using (var seed = fx.NewDbContext())
        {
            seed.GlossaryTerms.Add(
                new Mathom.Web.Domain.GlossaryTerm
                {
                    Id = Guid.NewGuid(),
                    UserId = u,
                    Term = "Obersaxen",
    ...
    public async Task Process_IncludesTermDescription_InCleanupGlossary()
    {
        ...
        await using (var seed = fx.NewDbContext())
        {
            var term = new Mathom.Web.Domain.GlossaryTerm
            {
                ...
                Term = "FireSkills",
                CreatedAt = System.DateTimeOffset.UtcNow,
                Description = "our internal time-tracking product",
            };
            term.Variants.Add(
                new Mathom.Web.Domain.GlossaryVariant
                {
        ...
        Assert.Contains(
            "FireSkills (also heard as: Fairstills) — our internal time-tracking product",
            llm.LastGlossary
        );
    }
    ...
    public async Task ProcessAsync_Photo_ReadsThenCleans()
    {
        ...
        string k1,
            k2;
        using (var a = new MemoryStream(tinyJpeg))
            k1 = await media.SaveAsync(a, ".jpg", CancellationToken.None);
        using (var b = new MemoryStream(tinyJpeg))
            k2 = await media.SaveAsync(b, ".jpg", CancellationToken.None);
    ...
    public async Task ProcessAsync_Photo_GeneratesVariant_AndFeedsItToReader()
    {
        ...
        byte[] originalBytes;
        using (
            var img = new SixLabors.ImageSharp.Image<SixLabors.ImageSharp.PixelFormats.Rgba32>(
                3000,
                2000
            )
        )
        ...
        string originalKey;
        using (var s = new MemoryStream(originalBytes))
        ...
        await using (var db = fx.NewDbContext())
        {
            var variants = new Mathom.Web.Media.PhotoVariantService(
                db,
                media,
                new Mathom.Web.Media.ImageVariantProcessor()
            );
        ...
        {
            var photo = await db
                .ItemPhotos.IgnoreQueryFilters()
                .FirstAsync(p => p.ItemId == itemId);
            Assert.False(string.IsNullOrEmpty(photo.DisplayPath));
            // The reader got the variant, not the original.
            using var variantStream = await media.OpenReadAsync(
                photo.DisplayPath!,
                CancellationToken.None
            );
            using var vms = new MemoryStream();
            await variantStream.CopyToAsync(vms);
    ...
    public async Task ProcessAsync_Photo_PassesGlossaryToReader()
    {
        ...
        string k;
        using (var a = new MemoryStream(tinyJpeg))
            k = await media.SaveAsync(a, ".jpg", CancellationToken.None);
    ...
    public async Task ProcessAsync_Photo_WithCaptureNote_PassesNoteAndPrepends()
    {
        ...
        var media = new FakeMediaStore();
        var tinyJpeg = await MakeTinyJpegAsync();
        var k1 = await media.SaveAsync(new MemoryStream(tinyJpeg), ".jpg", default);
