...
public class UserAdminService
{
    ...
    public UserAdminService(MathomDbContext db) => _db = db;

    public async Task<bool> IsApprovedAsync(string userId, CancellationToken ct)
        => await _db.Users.Where(u => u.Id == userId).Select(u => u.IsApproved).FirstOrDefaultAsync(ct);
    ...
    public async Task<bool> RevokeAsync(string actingUserId, string targetUserId, CancellationToken ct)
    {
        ...
        var u = await _db.Users.FirstOrDefaultAsync(x => x.Id == targetUserId, ct);
        if (u is null) return false;
        u.IsApproved = false;
        await _db.SaveChangesAsync(ct);
        return true;
