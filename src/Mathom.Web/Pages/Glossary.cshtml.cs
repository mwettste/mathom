...
[Authorize]
public class GlossaryModel(GlossaryService glossary) : PageModel
{
    ...
    public async Task OnGetAsync(CancellationToken ct)
        => Terms = await glossary.GetTermViewsAsync(UserId, ct);

    public async Task<IActionResult> OnPostAddAsync(string? term, string? variant, CancellationToken ct)
    {
        await glossary.AddAsync(UserId, term ?? string.Empty, variant, ct);
        Terms = await glossary.GetTermViewsAsync(UserId, ct);
